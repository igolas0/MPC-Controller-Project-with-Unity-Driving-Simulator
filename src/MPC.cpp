...
//Number N of timesteps
int N = 15;
//duration dt of elapsed time between timesteps
double dt = 0.1;

//Lf is the length from front to CoG that has a similar radius.
...
//reference velocity is set to 120 mph.
double ref_v = 120;

// The solver takes all the state variables and actuator
// variables in a singular vector. Setting up variable to refer to
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      // Implement the motion equations and CTE + orientation error:
      ...
      //change in sign for psi update equation
      //since simulator shows contrarian sign for delta_t
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //Defining state variables for readability
  double x = state[0];
  double y = state[1];
  double psi = state[2];
  double v = state[3];
  ...
  // Return the first actuator values (steering and throttle)
